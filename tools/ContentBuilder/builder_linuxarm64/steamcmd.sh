...
if [ -z "${STEAMROOT}" ]; then
  log "Couldn't find Steam root directory from $0, exiting!"
  exit 1
fi
...
if [ "$UNAME" == "Linux" ]; then
  ...
  log "Unknown system type $UNAME, exiting"
  exit 1
fi

if [ ! -e "$STEAMEXE" ]; then
  log "Couldn't find steamcmd at $STEAMEXE, exiting"
  exit 1
fi
...
if [ "$DEBUGGER" == "gdb" ] || [ "$DEBUGGER" == "cgdb" ]; then
  ...
  log Starting "$DEBUGGER" -x "$ARGSFILE" "$DEBUGGER_ARGS" --args "$STEAMEXE" "$@"
  $DEBUGGER -x "$ARGSFILE" "$DEBUGGER_ARGS" --args "$STEAMEXE" "$@"

  rm "$ARGSFILE"
else
  log Starting "$DEBUGGER" "$STEAMEXE"
  $DEBUGGER "$STEAMEXE" "$@"
fi
